...
# ---------------------------------------------------------------------------
# Path where the MCP (JSON-RPC) endpoint is exposed.
# This is the URL path ChatGPT / OpenAI tooling should call.
# We default to "/mcp" but allow override via env var.
...
def run_server() -> None:
    """Start the MCP server in the most compatible way.

    Why this exists:
    - Your Railway logs show 500 errors on /mcp with:
      "Task group is not initialized. Make sure to use run()."
      That happens when we run an ASGI app wrapper without FastMCP's internal
      runtime being started.

    Fix:
    - Start the server via `mcp.run(...)`, which initializes the internal task
      group and StreamableHTTP session manager.
    """

    # Local default is 8787 (nice for ngrok examples). Railway will override
    # via the PORT env var automatically.
    port = int(os.getenv("PORT", "8787"))

    # ChatGPT MCP expects Streamable HTTP. Different versions of FastMCP accept
    # slightly different transport names. We'll try a few.
    preferred = os.getenv("MCP_TRANSPORT", "streamable-http")
    transports = [preferred, "streamable-http", "streamable_http", "http"]
    transports = [t for i, t in enumerate(transports) if t and t not in transports[:i]]

    # Some versions accept `path=...`, others don't. We attempt with and without.
    path_kw_variants = [
        {"path": MCP_PATH},
        {"mount_path": MCP_PATH},
        {"endpoint": MCP_PATH},
    ...
    print("📋 14 DIAL Architectures loaded")
    print(f"🔗 MCP endpoint path: {MCP_PATH}")

    last_err: Exception | None = None
    for transport in transports:
        for extra in path_kw_variants:
            try:
                # Keep kwargs minimal for max compatibility.
                mcp.run(transport=transport, host="0.0.0.0", port=port, **extra)
                return
    ...
    raise RuntimeError(
        "Unable to start FastMCP server with available transports/arguments. "
